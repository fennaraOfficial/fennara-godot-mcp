...
These guidelines are generated by `fennara install`. They are intentionally detailed because MCP clients and coding agents need explicit rules to work safely inside Godot projects.
...
Runtime controls are intentionally low-level and game-agnostic. Agents must inspect the project's real controls and game code, then compose primitive runtime actions for that specific game. Detailed runtime_script syntax, ctx methods, strict typing requirements, and input-driving patterns live in the runtime_script tool schema.
