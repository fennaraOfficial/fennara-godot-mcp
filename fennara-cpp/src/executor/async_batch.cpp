...
bool complete_if_blocked(FennaraExecutor *executor,
    ...
    executor->_on_async_tool_complete(addon_block, tool_index, tool_name, args);
    return true;
}
...
void FennaraExecutor::execute_tool_calls_async(const godot::Array &tool_calls) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (name == "script_diagnostics") {
            ...
            tool.instantiate();
            tool->connect("complete", callable_mp(this, &FennaraExecutor::_on_async_tool_complete).bind(i, name, args));
            ...
            if (is_success && (file_path.ends_with(".gd") || file_path.ends_with(".cs"))) {
                ...
                _on_async_tool_complete(write_res, i, name, args);
            }
        } else if (name == "screenshot_scene") {
            if (complete_if_blocked(this, name, args, i, args.get("scene_path", ""))) {
                continue;
            }
            ...
            for (int scene_i = 0; scene_i < scene_paths.size(); scene_i++) {
                if (complete_if_blocked(this, name, args, i, scene_paths[scene_i])) {
                    blocked = true;
                    break;
                }
            }
            ...
            if (complete_if_blocked(this, name, args, i, args.get("script_path", ""))) {
                continue;
            }
            ...
            if (action == "start" &&
                complete_if_blocked(this, name, args, i, args.get("scene_path", ""))) {
                continue;
            }
            ...
            _on_async_tool_complete(res, i, name, args);
    ...
    if (has_batch_diagnostics) {
        ...
        _batch_diag_thread = std::thread(&FennaraExecutor::_run_batch_diagnostics, this);
    } else {
...
void FennaraExecutor::_on_async_tool_complete(const godot::Dictionary &result,
    ...
    if (_batch_cancelled) {
        ...
        ignored_context["reason"] = "batch_cancelled";
